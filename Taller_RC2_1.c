...
void menu_opciones() {
    ...
    printf("|4. Cálculo de promedios historicos        |\n");
    printf("|5. Generación de recomendaciones          |\n");
...
void acciones_zonas(const char *archive_csv, const char *zone_name){
    ...
    do{
        printf("\t\x1B[33m%s\x1B[0m\n", zone_name);
        menu_opciones();
        option = ingreo_numeroentero("Seleccione una acción", "Opción no válida, ingrese nuevamente", "Superó el límite, ingrese nuevamente", 1, 7);
        switch (option){
        case 1:
...
void monitoreo_contaminacion_zonas(zone_data *zone){
    printf("\t\t\x1B[33mMonitoreo de Contaminación de los ultimos %d dias en %s\x1B[0m\n", zone->num_data, zone->zone_name);
...
void prediccion_nivelesfuturo(zone_data *zone) {
    ...
    do{
        ...
        if (result_valid == 0){
            printf("\x1B[31mFecha inválida, debe ser en formato (dia/mes/anio) o revise el dia y mes\x1B[0m\n");
        } else if (result_valid == -1){
            printf("\x1B[33mFecha inválida, no se puede ingresar una fecha pasada\x1B[0m\n");
        } else if (result_valid == -2) {
            printf("\x1B[33mFecha inválida, no se puede ingresar una fecha tan distante\x1B[0m\n");
        } else {
            break;
        }
    ...
    printf("------------------------------------------------------------------\n");
    printf("|Contaminante\t| Nivel Actual\t\t|Predicción en 24 horas\t|\n");
    printf("------------------------------------------------------------------\n");
    printf("|PM 25\t\t| %.2f µg/m³\t\t| %.2f µg/m³\t\t|\n", prom_pm25, predic_pm25);
    printf("|PM 10\t\t| %.2f µg/m³\t\t| %.2f µg/m³\t\t|\n", prom_pm10, predic_pm10);
    printf("|O3\t\t| %.2f µg/m³\t\t| %.2f µg/m³\t\t|\n", prom_03, predic_o3);
    printf("|NO2\t\t| %.2f µg/m³\t\t| %.2f µg/m³\t\t|\n", prom_no2, predic_no2);
    printf("|SO2\t\t| %.2f µg/m³\t\t| %.2f µg/m³\t\t|\n", prom_so2, predic_so2);
    printf("|CO\t\t| %.2f mg/m³\t\t| %.2f mg/m³\t\t|\n", prom_co, predic_co);
    printf("------------------------------------------------------------------\n");
}
...
void alertas_preventivas(zone_data *zone) {
    ...
    for (int i = 0; i < zone->num_data; i++) {
        ...
        if (d->pm25 > 25 || d->pm10 > 50 || d->o3 > 100 || d->no2 > 200 || d->so2 > 20 || d->co > 10) {
            ...
            if (d->pm25 > 25) { 
                printf("  PM2.5: %.2f µg/m³ (Límite: 25)\n", d->pm25); 
                alerta = 1; 
            }
            if (d->pm10 > 50) { 
                printf("  PM10: %.2f µg/m³ (Límite: 50)\n", d->pm10); 
                alerta = 1; 
            }
            if (d->o3 > 100)  { 
                printf("  O3: %.2f µg/m³ (Límite: 100)\n", d->o3); 
                alerta = 1; 
            }
            if (d->no2 > 200) { 
                printf("  NO2: %.2f µg/m³ (Límite: 200)\n", d->no2); 
                alerta = 1; 
            }
            if (d->so2 > 20)  { 
                printf("  SO2: %.2f µg/m³ (Límite: 20)\n", d->so2); 
                alerta = 1; 
            }
            if (d->co > 10)   { 
                printf("  CO: %.2f mg/m³ (Límite: 10)\n", d->co); 
                alerta = 1; 
            }
            if (alerta) {
                printf(" Recomendación: Evite actividades al aire libre. Use mascarilla si es necesario.\n\n");
                found_alert = 1;
            }
        }
    }
    if (!found_alert) {
        printf("\x1B[32mNo se detectaron niveles de contaminación peligrosos en los últimos %d días.\x1B[0m\n", zone->num_data);
    }
}
...
void calcularPromedio(zone_data *zone){
    ...
    printf("\n\x1B[36m--- PROMEDIOS HISTÓRICOS EN %s ---\x1B[0m\n", zone->zone_name);
    printf("Periodo de días\n");
    printf("=============================================\n");
    printf("Contaminante\t\tPromedio\t\tLímite OMS\n");
    printf("------------------------------------------------------------\n");
...
void generar_recomendaciones(zone_data *zone) {
    printf("\n\x1B[36m==============================\n");
    printf("  Generación de recomendaciones\n");
    printf("==============================\x1B[0m\n");
    printf("Zona: %s\n", zone->zone_name);
    printf("==============================\n");
    ...
    for (int i = 0; i < zone->num_data; i++) {
        ...
        printf("------------------------------\n");
        if (d.pm25 > MAX_PM25) {
            printf("  * Alta concentración de PM2.5. Se recomienda usar mascarilla.\n");
            score += 2;
        }
        if (d.pm10 > MAX_PM10) {
        ...
        if (d.co > MAX_CO) {
            printf("  * CO alto. Mucha combustión cercana.\n");
            score += 2;
        }

        if (score >= 6) {
            printf("\x1B[41m>> ALERTA CRÍTICA: Calidad del aire muy peligrosa.\x1B[0m\n");
            ...
            printf("\x1B[103m>> ALERTA MODERADA: Precaución necesaria.\x1B[0m\n");
        ...
        if (strstr(d.date, "06/12")) {
            printf("  * Fiestas de Quito: Evite congestión y uso de vehículos privados. Use el trolebús o la Ecovía.\n");
        }
...
void exportar_datos(zone_data *zone) {
    ...
    if (file == NULL) {
        printf("\x1B[31mError: No se pudo crear el archivo de exportación\x1B[0m\n");
        return;
    }
 
    fprintf(file, "=============================================\n");
    fprintf(file, "REPORTE DE CALIDAD DEL AIRE - ZONA: %s\n", zone->zone_name);
    fprintf(file, "Total de días registrados: %d\n", zone->num_data);
    fprintf(file, "=============================================\n\n");
